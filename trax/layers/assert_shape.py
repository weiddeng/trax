...
def assert_shape(specification):
  ...
  a trax.base.Layer class. It uses the notation similar to einsum (Einstein
  ...
  Apart from the increased readability, it puts three levels of asserts on the
  function :first level is the number of input tensors and output tensors;
  second level is the rank of each tensor; third level are the sizes of each
  dimension in each tensor. The decorator inserts those asserts right before
  and right after 'forward' call.
  ...
  case is an ellipsis '...' or a star '*', which expand to arbitrary number of
  dimensions, including zero. For example, the tensor with specification '*sf'
  has at least two. Each tensor may have in its representation either a single
  star, a single ellipsis, or neither of them.

  Third level, asserts on the sizes of each dimension. If two dimensions in any
  ...
  If two distinct tensors have an ellipsis, or a star, in their representation
  then all of dimensions covered by those ellipses must match. For example,
  with a tensor E represented by '...d' and tensor F represented by '...x'
  then E and F must have the same rank, and the sizes of all but the last
